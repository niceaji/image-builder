...
MEME.MemeEditorView = Backbone.View.extend({
  ...
  buildForms: function() {
    ...
    if (d.canvasSizeOpts && d.canvasSizeOpts.length) {
      $('#canvas-size').append(buildOptions(d.canvasSizeOpts)).show();
    }
  ...
  render: function() {
    ...
    this.$('#canvas-size').val(d.canvasSize);
  ...
  events: {
    ...
    'change #canvas-size': 'onCanvasSize',
    ...
    'click [name="colorType"]': 'onColorType'
  },

  onColorType: function() {
    this.model.set('colorType', this.$('[name="colorType"]:checked').val() );
  },
  onCredit: function() {
  ...
  onCanvasSize: function() {
    this.model.set('canvasSize', this.$('#canvas-size').val());
  },
